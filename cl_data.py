...
def make_compute_all_serie(repository_name, NSIDE, Bin, photoz_min, photoz_max, nshells, nmocks):
	...
	for i in range(nmocks):
		...
		if i < 10:
			catalog_name = repository_name + 'HALOGENlamps_V2.0.0_DNF_mock000' + str(i) + '_masked.dat'
		elif i >= 10 and i < 100:
                        catalog_name = repository_name + 'HALOGENlamps_V2.0.0_DNF_mock00' + str(i) + '_masked.dat'
		elif i >= 100 and i < 1000:
                        catalog_name = repository_name + 'HALOGENlamps_V2.0.0_DNF_mock0' + str(i) + '_masked.dat'
		else:
                        catalog_name = repository_name + 'HALOGENlamps_V2.0.0_DNF_mock' + str(i) + '_masked.dat'
		...
		for j in range(nshells):
			...
			f0 = nmt.NmtField(mask, [dmap])
		        ...
		        #dir_name = os.getcwd()
			#output_name = '/home/drc01/sobreira/andluiz/halogen/outputs/cldata/cl_mock' + str(i) + '_shell'  + str(j) + '.dat '
...
def make_compute_all_parallel(repository_name, NSIDE, Bin, photoz_min, photoz_max, nshells, nmocks):
	...
	for i, task in enumerate(task_list):
		...
		make_compute_single_parallel(repository_name, mask, wsp, NSIDE, photoz_min, photoz_max, nshells, Bin, task)
...
def make_compute_single_parallel(repository_name, mask, wsp, NSIDE, photoz_min, photoz_max, nshells, Bin, i):
...
def binning(lmin, lmax, dl):
	...
	#name_output = '/home/drc01/sobreira/andluiz/halogen/outputs/bin.dat'
        #dir_name = os.getcwd()
	name_output = dir_name + '/outputs/bin.dat'
...
def make_plots(Bin, nshells, nmocks, bias):
	...
	ell_fid = np.loadtxt(dir_name + '/outputs/cltheo.dat', usecols=(0), unpack=True)

	for i in range(nshells):

		print 'Plotting Cls shell ' + str(i+1)

		cl_fid =  np.loadtxt(dir_name + '/outputs/cltheo.dat', usecols=(i+1), unpack=True)*(bias[i]**2)

                plt.pyplot.figure(figsize = (6,5), dpi = 1024)
                plt.pyplot.semilogy(ell, cl_mean[i,:], 'r.', label = "NaMaster data")
                plt.pyplot.semilogy(ell_fid, cl_fid, 'b-', label = "External data")
...
def make_covmat(Bin, nshells, nmocks):
...
def start():
	...
	# MASK INPUT
        repository_name = '/home/drc01/sobreira/andluiz/halogen/mock_cats/'
	mask_name = repository_name + 'Y1redLSSmaskv1.04096ring.dat'
	#make_mask(mask_name, NSIDE)
	...
	# BINNING ELLS'S INPUTS
        ...
        #bpws, ells, weights = binning(lmin, lmax, dl)

	# Or reading binning scheme
	bin_name = dir_name + '/bin.dat'
	...
	#make_compute_all_serie(repository_name, NSIDE, Bin, photoz_min, photoz_max, nshells, nmocks)
	#make_compute_all_parallel(repository_name, NSIDE, Bin, photoz_min, photoz_max, nshells, nmocks)


	# GALAXY-BIAS FOR COMPARATION WITH EXTERNAL DATA
	bias = ([1.83, 1.77, 1.79, 2.15])

	# MAKING GHAPH PLOTS
	make_plots(Bin, nshells, nmocks, bias)

	# COMPUTING COVARIANCE MATRIX
	make_covmat(Bin, nshells, nmocks)
